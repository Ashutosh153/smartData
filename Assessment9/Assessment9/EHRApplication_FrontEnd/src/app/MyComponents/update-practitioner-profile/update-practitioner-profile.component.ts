...
@Component({
  selector: 'app-update-practitioner-profile',
  standalone: true,
  imports:  [CommonModule,ReactiveFormsModule,RouterLink],
  templateUrl: './update-practitioner-profile.component.html',
  styleUrl: './update-practitioner-profile.component.css'
})
export class UpdatePractitionerProfileComponent {
    ...
    if(this.imageFile)
      {
        ...
        this.service.DoAddImage(formData).subscribe({
          next:(res:any)=>
          {
           ...
           specialisation_ID:this.profileForm.get('Specialisation_ID')?.value||0,
           registration_Number:this.profileForm.get('Registration_Number')?.value||'',
           visiting_Charge:this.profileForm.get('Visiting_Charge')?.value||0,
    ...
    this.service.DoGetUserById(Id).subscribe({
      ...
      next:(res:any)=>{
        ...
        if(res.isSuccess)
        {
          ...
          console.log("jkjebjbefjewbfj  uwegew ",res)
          ...
          // this.profileForm.get('Email')?.setValue(res.data.email)
           ...
           this.profileForm.get('id')?.setValue(res.data[0].id)
        ...
        //  this.profileForm.get('DOB')?.setValue(res.data[0].dateOfBirth)
        this.profileForm.get('DOB')?.setValue(this.LoginUser.dateOfBirth)
          ...
          this.profileForm.get('qualification')?.setValue(res.data[0].qualification)
          this.profileForm.get('Specialisation_ID')?.setValue(res.data[0].specialisation_ID)
          this.profileForm.get('Registration_Number')?.setValue(res.data[0].registration_Number)
          this.profileForm.get('Visiting_Charge')?.setValue(res.data[0].visiting_Charge)
...
saveUser(Payload:any)
{
  debugger
  console.log(Payload)
  this.service.DoUpdateProvider(Payload).subscribe((res:any)=>{
